...
class Pagina:
    ...
    def setMenuAdm(self, dicionario, usuario = ''):
        ...
        </span> Olá %s </a></li><li><a href='/altera_senha'> Alterar senha</a></li><li class='text-success'><a href='/logout'> \
...
def selecaoPer(nome, variavel, lista, listaSelecionados = [], classe = '', Identificador = 'lojas', uni = False):
    ...
    '''
    sele = "%s<select name='%s' multiple class='form-control %s' id='%s'>" % (str(nome), str(variavel), str(classe), str(Identificador))
    filiais = ''
...
class Tabela:
    ...
    def getTabela(self):
        ...
        if isinstance(self.__corpo, list) and isinstance(self.__cabecalho, list) and isinstance(self.__rodape, list):
            ...
            tab = "<div class='tabe table-responsive small'><table class='table text-center table-hover table-bordered table-responsive small' id='minhaTabela'><thead><tr class='success'>%s</tr></thead><tbody>%s</tbody><tfoot><tr class='rodape success'>%s</tr></tfoot></table></div>"
            # Gerando cabecalho
            tcabe = "<th>%s</th>"
            cabe = ''
...
class Vendedor:
    ...
    def __init__(self, nome, id_vendedor, imagem, empresa = '', filial = ''):
        ''' Instancia um objeto vendedor recebendo nome, id imagem empresa e filial'''
        self.__nome = nome
        self.__id_vendedor = id_vendedor
        self.__imagem = '/imagens/sem_foto.png' if imagem is None or imagem == "" else imagem
    ...
    def setIdvendedor(self, id_vendedor):
    ...
    def getImagem(self):
        ''' Retorna a imagem do vendedor '''
        return self.__imagem

    def setFilial(self, filial):
        ''' Define a filial do vendedor'''
        self.__filial = filial

    def getFilial(self):
        ''' Obtem a filial do vendedor '''
        return self.__filial

    def retornaVendedor(self):
        ''' Retorna uma representaao de uma tag img com id do vendedor e pronto para o dragdrop '''
        img = '<p id="'+str(self.getIdVendedor())+'" title="'+self.getNome()+'"><img style="width:48px;height:48px" title="'+self.getNome()+'" class="img img-circle" src="'+self.getImagem()+'" alt="'+self.getNome()+'" />'+self.getNome().split(' ')[0]+' <time class="text-danger" style="font-weight:bold"></time></p>'
        return img