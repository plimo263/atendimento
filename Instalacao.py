...
try:
	...
	print('Uma das bibliotecas não esta presente: %s' % str(err))
	quit()
...
def instalar():
	...
	try: # Tentando se conectar a base de dados
		...
		print('Não foi possível se conectar ao banco de dados %s, ele existe ? :%s' % (config.MY_BANCO, str(err)));
		print('Abortando ...')
		quit()
	except pymysql.err.OperationalError as err:
		print('Não foi possível se conectar ao SGBD ou o usuario não tem acesso ao banco %s: %s' % (config.MY_BANCO, str(err)))
		print('Abortando ...')
		quit()
	time.sleep(2)
	...
	os.chdir('banco_de_dados')
	with open('base.sql', encoding = 'utf-8') as arq:
		for f in arq.readlines():
			try:
				...
				print('Possível erro de sintaxe, reportar erro pelo email plimo263@gmail.com: %s ' % str(err))
			# Quando a tabela existe, preciso que o usuario diga se deseja sobreescreve-la
				...
				resp = input('Possívelmente a tabela %s já existe, deseja sobreescreve-la ? S/N ' % tabe)[0]
				if resp.upper() == 'S':
...
for para in parametros:
	if not para in dir(config):
		print('O parametro %s não existe abortando ...' % para)
		quit()
	if para == 'MY_PORTA' and not isinstance(config.MY_PORTA, int):
		print('A variavel MY_PORTA em config deve ser um inteiro.')
		print('Abortando ...')
		quit()
